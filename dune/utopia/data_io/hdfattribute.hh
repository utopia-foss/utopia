 ...
 */
template <typename HDFObject> class HDFAttribute {
private:
...
public:
   ...
   * @param adaptor
   */
  template <typename Type> auto read() {
    if (_attribute == -1) {
   ...
   * @param adaptor
   */
  template <typename Type> void write(Type &attribute_data) {
  ...
  virtual ~HDFAttribute() {
    ...
    if (H5Iis_valid(_attribute) == 0) { // FIXME: add check to make sure that
                                        // it is not
