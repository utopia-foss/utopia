...
private:
  ...
  hid_t __make_dataset_compressed__(hsize_t chunksize, hsize_t compress_level) {
...
public:
  ...
  void write(Iter begin, Iter end, Adaptor &&adaptor, hsize_t rank = 1,
    ...
    if (_rank > 2) {
      throw std::runtime_error("ranks higher than 2 not supported currently");
    }
    ...
    if (_dataset == -1) { // dataset does not exist
      if (chunksize > 0) {
        ...
        if (compress_level > 0) {
          ...
          _dataset = __make_dataset_compressed__<result_type>(chunksize,
                                                              compress_level);
        } else {
      ...
      herr_t write_err = H5Dwrite(_dataset, HDFTypeFactory::type<result_type>(),
                                  H5S_ALL, H5S_ALL, H5P_DEFAULT, buffer.data());
      ...
      if (_rank == 1) {
        ...
        */
        ...
        if (ext_err < 0) {
          throw std::runtime_error("1d dataset could not be extended ");
        }
        ...
        herr_t select_err = H5Sselect_hyperslab(dspace, H5S_SELECT_SET, &offset,
                                                &stride, &size, &block);
        if (select_err < 0) {
  ...
  std::vector<result_type> read(std::vector<hsize_t> start = {},
    ...
    if (_rank == 1) {
      ...
      if (start.size() == 0) {
        ...
        herr_t read_err = H5Dread(_dataset, type, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                                  buffer.data());
        if (read_err < 0) {
        ...
        H5Sclose(memspace);
        if (read_err < 0) {
          throw std::runtime_error("Error reading subset of 1d dataset");
        }
        return buffer;
      }
    } else { // N D dataset
  ...
  HDFDataset(HDFObject &parent_object, std::string name)
      : _parent_object(std::make_shared<HDFObject>(parent_object)), _name(name),
