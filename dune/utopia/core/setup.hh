...
namespace Setup
{
    ...
    */
    ...
    /** \param name          The name of the model instance
      ...
      */ 
    template<bool periodic=true, unsigned short dim=2, bool structured=true,
             bool sync=true,
             typename ParentModel, typename State>
    auto create_grid_manager_cells(const std::string name,
                                   const ParentModel& parent_model,
                                   const State initial_state)
    {
        // Get the grid, passing through arguments
        auto grid = create_grid_from_model(name, parent_model);
