...
void test_model_construction(Model& model)
{
    ...
    std::vector<double> init_cell_resources =
        as_vector<double>(cfg["init_cell_resources"]);
    ...
    double deaththreshold = as_double(cfg["deaththreshold"]);
    ...
    for (auto& cell : cells)
    {
        ...
        else
        {
            ...
            ASSERT_EQ(state.resources, init_cell_resources);
...
// template <typename Model>
// void test_model_integration(Model& model)
// {
//     auto& agents = model.agents();
//     auto& cells = model.cells();
//     auto as = agents.size();
//     auto cs = cells.size();
//     std::cout << as + cs << std::endl;
// }
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        Utopia::PseudoParent<RNG> parentmodel_simple("test_config_simple.yml");
        Utopia::PseudoParent<RNG> parentmodel_complex(
            "test_config_complex.yml");
        Utopia::PseudoParent<RNG> parentmodel_highlevel(
            "test_config_highlevel.yml");
        ...
        auto agents_simple =
            Utopia::Setup::create_agents_on_grid(wrapper_simple, 1, ASS());
        ...
        Utopia::GridWrapper<GridType> wrapper_complex{
            grid_complex, cellmanager_complex.extensions(),
            cellmanager_complex.grid_cells()};
        ...
        // get AmeeMulti typedef
        using AmeeMulti_HL =
            AmeeMulti<Cell, decltype(cellmanager_highlevel),
                      decltype(agentmanager_highlevel), Modeltypes_HL, true, true>;
