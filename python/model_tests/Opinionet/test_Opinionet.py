...
def test_dynamics():
    ...
    for uni_no, uni in dm['multiverse'].items():
        ...
        if ('opinion_space'!='discrete' and cfg['interaction_function'] != 'Deffuant'):

            assert all(np.abs(val - final_opinions[0]) < 0.05
...
def test_rewiring():
    ...
    for uni_no, uni in dm['multiverse'].items():
        ...
        assert (all(len(edges_initial[i]) == len(edges_final[i]))
            for i in range(len(edges_final)))

        # Check edges have changed
        assert (all(edges_initial[i] != edges_final[i])
            for i in range(len(edges_final)))

        # Get intial and final graphs
        nw_init = GraphPlot.create_graph_from_group(data['nw'], at_time_idx=0)
        nw_final = GraphPlot.create_graph_from_group(data['nw'], at_time_idx=-1)
