...
def test_output():
    ...
    _, dm = mtc.create_run_load(
        from_cfg="state_output.yml", perform_sweep=True
    )
    ...
    for uni_no, uni in dm["multiverse"].items():
        ...
        assert "some_global_parameter" in data
        assert "some_heterogeneous_parameter" in data
...
def test_slope():
    ...
    for uni_no, uni in dm["multiverse"].items():
        ...
        true_data = np.empty(data.grid_shape)
        true_data[:, 0].fill(0.0)
        true_data[:, 1].fill(0.0)
        true_data[:, 2].fill(0.5)
        true_data[:, 3].fill(0.5)
        true_data[:, 4].fill(1.0)
        true_data[:, 5].fill(1.0)
...
def test_noise():
    ...
    for uni_no, uni in dm["multiverse"].items():
        ...
        assert np.isclose(
            data.sel({"time": 0}).mean(dim=["x", "y"]), [1.0], atol=1.0e-2
        )
        assert np.isclose(
            data.sel({"time": 1}).mean(dim=["x", "y"]), [0.5], atol=5.0e-2
        )

        assert np.isclose(
            data.sel({"time": 2}).mean(dim=["x", "y"]), [0.0], atol=5.0e-2
        )
        assert np.isclose(
            data.sel({"time": 2}).std(dim=["x", "y"]), [1.0], atol=1.0e-2
        )

        assert np.isclose(
            data.sel({"time": 3}).mean(dim=["x", "y"]), [10.0], atol=5.0e-2
        )
        assert np.isclose(
            data.sel({"time": 4}).mean(dim=["x", "y"]), [20.0], atol=5.0e-2
        )
