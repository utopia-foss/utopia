...
def model_cfg(**kwargs) -> dict:
    """Creates a dict that can update the config of the PredatorPrey model and the parameter space"""
...
def test_basics():
    ...
    # Create a Multiverse using the default model configuration, except for smaller grid size and one step
...
def test_initial_state_random(): 
    ...
    mv, dm = mtc.create_run_load(from_cfg="initial_state.yml",
                                 perform_sweep=True,
                                 **model_cfg(initial_state='random', prey_prob=prey_prob, pred_prob=pred_prob))
    ...
    for uni in dm['uni'].values():
        ...
        assert data['resource_pred'].shape == (1, num_cells)

        # Number of cells should be prey + predators + empty, every cell should either be empty or populated by either predator or prey
        ...
        assert num_predator == np.sum(data['resource_pred']) / 2
...
def test_initial_state_fraction():
    ...
    mv, dm = mtc.create_run_load(from_cfg="initial_state.yml",
                                 ...
                                 **model_cfg(initial_state='fraction',
                                             prey_frac=prey_frac, pred_frac=pred_frac))
    ...
    for uni in dm['uni'].values():
        ...
        assert num_prey == int(prey_frac * data['Population'].shape[1])  # floor
        assert num_pred == int(pred_frac * data['Population'].shape[1])  # floor
