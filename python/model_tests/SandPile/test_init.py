...
def test_initial_datasets():
    ...
    mv, dm = mtc.create_run_load(
        from_cfg="initial_datasets.yml", perform_sweep=True
    )
...
def test_initial_no_toppling():
    ...
    mv, dm = mtc.create_run_load(
        from_cfg="initial_no_toppling.yml", perform_sweep=True
    )
    ...
    for uni in dm["multiverse"].values():
        ...
        test_avalanche_size_data = xr.DataArray(
            [1], dims=["time"], coords=dict(time=time_coords)
        )
        ...
        xr.testing.assert_allclose(data["slope"].data, test_slope_data, atol=1)
        xr.testing.assert_allclose(
            data["avalanche"].data, test_avalanche_data, atol=0.5
        )
        xr.testing.assert_equal(
            data["avalanche_size"].data, test_avalanche_size_data
        )
...
def test_initial_toppling():
    ...
    mv, dm = mtc.create_run_load(
        from_cfg="initial_toppling.yml", perform_sweep=True
    )
    ...
    for uni in dm["multiverse"].values():
        ...
        test_slope_data = xr.DataArray(
            [resolution * [resolution * [critical_slope / 2.0]]],
            dims=["time", "x", "y"],
            coords=dict(time=time_coords, x=space_coords, y=space_coords),
        )
        ...
        # of cells.
        test_avalanche_size_data = xr.DataArray(
            [resolution**2], dims=["time"], coords=dict(time=time_coords)
        )
        ...
        xr.testing.assert_allclose(data["avalanche"].data, test_avalanche_data)
        xr.testing.assert_equal(
            data["avalanche_size"].data, test_avalanche_size_data
        )
...
def test_invalid_parameters():
    ...
    with pytest.raises(SystemExit):
        mtc.create_run_load(
            from_cfg="initial_invalid_argument.yml",
            perform_sweep=False,
            **cm_cfg(
                cell_params=dict(
                    initial_slope_lower_limit=6, initial_slope_upper_limit=5
                )
            ),
        )
    ...
    with pytest.raises(SystemExit):
        mtc.create_run_load(
            ...
            **cm_cfg(
                cell_params=dict(
                    initial_slope_lower_limit=5, initial_slope_upper_limit=5
                )
            ),
        )
    ...
    with pytest.raises(ValidationError):
        mtc.create_run_load(
            ...
            **cm_cfg(
                cell_params=dict(
                    initial_slope_lower_limit=-5, initial_slope_upper_limit=-1
                )
            ),
        )
