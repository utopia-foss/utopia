...
def draw_graph(*, hlpr: PlotHelper,
    ...
    Args:
        hlpr (PlotHelper): PlotHelper
        data (dict): Data from DAG selection
        graph_cfg (dict): Configuration for graph creation, layout and animation
    ...
    def select_from_list(g, *, nodes: list):
        """Given a list of selected nodes, selects all edges for which both ends
        are in the set of selected nodes.
    ...
    def select_radial(g, *, center: int, radius: int):
        ...
        (measured in numbers of neighborhoods). Selects all edges for which both
        ...
        # Create a (read-only) GraphView of the subgraph induced by the selected
        # nodes, i.e., a graph containing the selected nodes N and all edges
        # that have both ends in N.
            ...
            if isinstance(g_prop, dict) and g_prop.get('from_property', False):
                ...
                if plt_prop == 'node_size' and prop == 'degree':
                    ...
                    node_kwargs['node_color'] = scale_to_interval(
                            [g.nodes[n][prop] for n in nodelist], interval)
                    show_cbar = node_kwargs.pop('colorbar', True)
        ...
        if label_kwargs.get('labels', False):
            ...
            if label_kwargs['labels'].get('from_property', False):
                ...
                if show_only_given:
                    ...
                    # show_only. nodes in show_only for which no label was given
                    # are labeled with their index.
    ...
    def parse_edge_label_kwargs(g, *, edgelist: list, label_kwargs: dict):
        ...
        if label_kwargs.get('edge_labels', False):
            ...
            if label_kwargs['edge_labels'].get('from_property', False):
                ...
                # If the graph allows parallel edges, they must be three-tuples.
                ...
                if any([len(e)!=edge_tuple_length for e in show_only]):
                    raise TypeError("Received invalid edge(s) in 'show_only' "
                                    "for graph of type {}: {}. Expected tuples "
                                    "of size {}."
                                    "".format(type(g),
                ...
                if show_only_given:
                    # If some labels are given, keep only those that are in
                    # show_only. edges in show_only for which no label was given
                    # are labeled with their (source, destination) pair.
                    for e in list(label_kwargs['edge_labels'].keys()):
                        # Delete edge label entries that are not in show_only
                        if all([(e!=edge and e!=edge[:2])
                                for edge in show_only]):
                            del label_kwargs['edge_labels'][e]
                    for e in show_only:
                        if e[:2] not in list(label_kwargs['edge_labels'].keys()):
    ...
    def prepare_and_plot(graph_group, *,
        ...
        # Create the graph from the GraphGroup. The Graph object is created once
        # and not changed in the following.
        ...
        # NOTE This also means that edges can only be drawn if both their source
        #      and destination are drawn.
    ...
    # Hide the axes
    # FIXME This should be done by a `PlotHelper` method, and hence be
    #       configurable.
    hlpr.ax.axis('off')
    ...
    def update():
        ...
        arange the nodes very differently in each iteration, even for only small
