...
def state_time(dm: DataManager, *, out_path: str, uni: int, fmt: str=None, save_kwargs: dict=None, **plot_kwargs):
    ...
    plt.plot(G_data[:,0], "green",label="grass")
    plt.plot(S_data[:,0], "yellow",label="sappling")
    plt.plot(T_data[:,0], "orange",label="tree")
    plt.plot(F_data[:,0], "red",label="forest tree")
    ax.legend()
...
def state_trajectory(dm: DataManager, *, out_path: str, uni: int, fmt: str=None, save_kwargs: dict=None, **plot_kwargs):
    ...
    for f in F_data[:,0]:
        if (f != 0):
            forest_exists = True
    if (forest_exists):
        plt.scatter(G_data[:,0],T_data[:,0],F_data[:,0],c=range(G_data.shape[0]),cmap=plt.cm.viridis)
        plt.scatter(G_data[:,0],T_data[:,0],np.zeros(G_data.shape[0]),c=range(G_data.shape[0]),cmap=plt.cm.viridis,alpha=0.3)
        plt.scatter(G_data[:,0],np.ones(G_data.shape[0]) ,F_data[:,0],c=range(G_data.shape[0]),cmap=plt.cm.viridis,alpha=0.3)
        plt.scatter(np.zeros(G_data.shape[0]),T_data[:,0],F_data[:,0],c=range(G_data.shape[0]),cmap=plt.cm.viridis,alpha=0.3)
        ax.set_xlabel("Grass")
        ax.set_ylabel("Trees")
        ax.set_zlabel("Forest")
    else:
        plt.scatter(G_data[:,0],S_data[:,0],T_data[:,0],c=range(G_data.shape[0]),cmap=plt.cm.viridis)
        plt.scatter(G_data[:,0],S_data[:,0],np.zeros(G_data.shape[0]),c=range(G_data.shape[0]),cmap=plt.cm.viridis,alpha=0.3)
        plt.scatter(G_data[:,0],np.ones(G_data.shape[0]) ,T_data[:,0],c=range(G_data.shape[0]),cmap=plt.cm.viridis,alpha=0.3)
        plt.scatter(np.zeros(G_data.shape[0]),S_data[:,0],T_data[:,0],c=range(G_data.shape[0]),cmap=plt.cm.viridis,alpha=0.3)
        ax.set_xlabel("Grass")
...
# def state_final(dm: DataManager, *, out_path: str, uni: int, fmt: str=None, save_kwargs: dict=None, **plot_kwargs):
    ...
    # F_data = grp['density_F'].reshape(num_steps+1, grid_size[0], grid_size[1])
    # S_data = np.empty(G_data.shape)
    # for i in range(G_data.shape[0]):
    #     for j in range(G_data.shape[1]):
    #         S_data[i][j] = 1-G_data[i][j]-T_data[i][j]-F_data[i][j]
