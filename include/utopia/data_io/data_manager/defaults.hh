...
template<typename Model>
using DefaultBuilder = std::function<
    std::shared_ptr<HDFDataset<HDFGroup>>(std::shared_ptr<HDFGroup>&, Model&)>;
...
struct DefaultDecider {
    ...
    DefaultDecider& operator=(const DefaultDecider&) = default;
    DefaultDecider& operator=(DefaultDecider&&) = default;

    /// Construct the decider from a configuration
    DefaultDecider(const Config&)
      : DefaultDecider()
    {}

    /// Construct the decider with a given functor
    DefaultDecider(std::function<bool(Model&)> f)
      : func(f)
    {}
...
struct SliceDecider : DefaultDecider<Model>
{
    ...
    SliceDecider& operator=(const SliceDecider&) = default;
    SliceDecider& operator=(SliceDecider&&) = default;
...
struct AlwaysDecider : DefaultDecider<Model>
{
    ...
    AlwaysDecider()
    : Base([](Model&) -> bool { return true; })
    {}

    AlwaysDecider(const Config&)
      : Base([](Model&) -> bool { return true; })
    {}
...
struct CompositeDecider : DefaultDecider<Model>
{
    ...
    CompositeDecider(const Config& cfg)
    {
        ...
        boost::hana::for_each(
            boost::hana::zip(configs, held_deciders),
            [](auto& conf_dcd_pair) {
...
struct SimpleCompositeDecider : DefaultDecider<Model>
{
    ...
    SimpleCompositeDecider& operator=(const SimpleCompositeDecider&) = default;
    SimpleCompositeDecider& operator=(SimpleCompositeDecider&&) = default;
