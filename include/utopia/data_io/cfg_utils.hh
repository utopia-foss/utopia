 ...
 * These files are available as so called "config nodes",
 ::Utopia::DataIO::Config objects, which are hierarchically nested maps,
 sequences, and scalars.
 * When retrieving values from these nodes, a type cast becomes necessary. While
 the library used for YAML reading provides such a function, we found it more
 convenient to define some thin wrappers around them, which supply more
 information when something goes wrong.
 * To remember the order of arguments, this sentence can be employed: ``get_as``
 a ``double``: the entry named ``my_double`` from this ``cfg`` node.
 * Of course, YAML has no types associated with each entry; the ::Utopia::get_as
 function *tries* to do this conversion and, if it fails, throws an exception.


 * \section impl Implementation
 * The core of this module consists of the ::get_as function, which in
 conjunction
 * with ::improve_yaml_exception works to improve error messages and simplifies
 * the syntax of converting yaml nodes to c++ types.
...
/** 
 * @brief This function is a wrapper around the yaml-cpp YAML::Node::as function and
 *  enhances the error messages that can occur in a read operation.
 * @detail 
 * Example:
 * \code{.cc}
