...
However, to *handle* that data, one expects data with the dimensions ``x``, ``y``, and ``time``; the :py:class:`~utopya.datacontainer.GridDC` takes care of reshaping that data in this way.
...
More on that can be found `here <https://xarray.pydata.org/en/stable/dask.html>`_.
