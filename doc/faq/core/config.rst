...
The ``PseudoParent`` takes care of loading that configuration file.

When you instantiate your model, you pass it a name and a reference to the ``PseudoParent`` instance.
...
The ``Utopia::Model`` constructor supports an optional ``custom_cfg`` argument.
If that argument is given, the custom configuration node is used instead of the one that is being extracted from the parent model.
