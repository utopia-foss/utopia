...
Via ``utopya``, you have full access to all of Utopia also in an interactive fashion, e.g. in an `IPython <https://ipython.org/install.html>`_ session or in a `Jupyter <https://jupyter.org>`_ Notebook.
...
The ``Model`` class
^^^^^^^^^^^^^^^^^^^
Objects of this class are your main tool to work with Utopia interactively.
When you want to run a certain model, say ``ForestFire``, the first thing you do is to create an instance of the ``Model`` class.
...
It basically manages the model invocation and loading and has the same capabilities as the CLI has; in fact, the CLI is just a wrapper around the ``Model`` class.
...
This will create a ``Multiverse`` (see below), run a simulation (here: with default parameters), and then load the data.
...
By using the ``Model`` class and its interface, you don't need to care about passing this info bundle around.
...
After that, it provides the interface to an associated ``DataManager`` and ``PlotManager`` which let you load the data and create plots, respectively.

If you want to create a ``Multiverse`` object from your existing ``Model`` object, use the ``Model.create_mv`` method (see above for documentation).
...
The ``FrozenMultiverse`` is just what you need. It couples to some output directory and then you can continue working with the ``DataManager`` and ``PlotManager`` just as you would directly after a simulation run.
...
.. note::
  ...
  If, on the other hand, you do not *want* the simulation data saved, the ``use_tmpdir`` argument might be of interest to you.
...
If you do not specify any other parameters, this will inspect the output directory of the chosen model and couple to the run with the latest timestamp. (See ``FrozenMultiverse.__init__`` for available arguments.)
...
.. note::
  ...
  Utopia requires to work with multi-dimensional data. As numpy can be very confusing in this way, the `xarray <http://xarray.pydata.org/en/stable/>`_ package is used for almost all of the data handling.
  It is highly recommended to have a look at how to work with labelled dimensions and coordinates; it will make your life much easier!
